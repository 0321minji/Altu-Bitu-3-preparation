...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            im0 = cv2.resize(im0, (500, 400)) #이거 안드로이드 스튜디오에서 받을때 화면이 잘려서 일단 넣어둔거임, 최적 비율 찾아서 조절해야할듯
            (flag, encodedImage) = cv2.imencode(".jpg", im0)
            # if seen == 55:
            #     alarm=1
            # if(names[int(cls)]=='Person drowning'):
            #     drowning_frame+=1
            if seen>1 and (drowning_frame/seen)>=0.1:
                alarm=1
                seen=0
                drowning_frame=0
                alarm_flag=True
            if alarm_flag and seen>5:
                #알람이 띄워지고 5프레임 이상 지났으면
                alarm=0
                alarm_flag=False
                #이거 안해줬더니 알림창 못 끔 ㅜㅜ 계속 새로 띄워서
