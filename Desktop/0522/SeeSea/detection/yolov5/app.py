...
def yolo_detect():
    yolo_run(weights=ROOT / '640510best.pt',  # model path or triton URL
        source=ROOT / 'data1.mp4',  # file/dir/URL/glob/screen/0(webcam)
        data=ROOT / 'data/coco128.yaml',  # dataset.yaml path
        imgsz=(640, 640),  # inference size (height, width)
        conf_thres=0.25,  # confidence threshold
...
# def video():
#    return Response(yolo_detect(), mimetype='multipart/x-mixed-replace; boundary=frame')
#     # with open('./cat2.jpg', 'rb') as f:
#     #     image_data = f.read()
#     # stream = BytesIO(image_data)

#     # Return the BytesIO object as a response
#     # return send_file('./cat2.jpg', mimetype='image/jpeg', as_attachment=False)
#     # return render_template('main.html', data='./cat2.jpg')
